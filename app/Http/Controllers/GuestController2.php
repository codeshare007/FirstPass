...
class GuestController extends Controller
{
    ...
    private function createTimeSlot($interval, $start_time, $end_time)
    {
    ...
    private function getTimeSlots($start_date, $start_time, $end_time, $time_interval, $boobkly_appts, $hour) {
        $slots = array();        
        $end_time = strtotime($start_date.' '.$end_time);
        do {
            $overlaped = null;
            $start_time = strtotime($start_date.' '.$start_time);
            print_r($time_interval); die;
            ...
            if ($overlaped) {
                ...
                $start_time = $overlaped + $time_interval;
            } else {
                // array_push($slots, [
                //     'start' => $start_time,
                //     'end' => date('h:i a', $start_time + $hour * 3600),
                //     'is_blocked' => false
                // ]);
                // $slots[] = [
                //     'start' => $start_time,
                //     'end' => date('h:i a', $start_time + $hour * 60),
                //     'is_blocked' => false
                // ];
            ...
            if ($get_times) {
                ...
                if (is_array($av_time)) {
                    ...
                    foreach ($av_time as $v) {                        
                        ...
                        // $slots[] = getSlots($start_time, $end_time, $time_interval, $boobkly_appts);
                        ...
                        // $slots = array_merge($slots, $this->getTimeSlots($start_date, $start_time, $end_time, $time_interval, $boobkly_appts, $request->hour));

                        // foreach ($slots as $i => $slot) {
                        //     if ($request->hour == 2) {
                        //         $add_time = 120 - $time_interval;
                        //     }else{
                        //         $add_time = 60 - $time_interval;
                        //     }
                        //     $endtime = strtotime('+'.$add_time.' minutes',strtotime($start_date.' '.$slot['end']));
//                            if ($request->hour == 2){
//                                $endtime = strtotime('+105 minutes',strtotime($start_date.' '.$slot['end']));
//                            }
//                            else{ $endtime = strtotime('+45 minutes',strtotime($start_date.' '.$slot['end'])); }
                           // $endtimestring = date('h:i a', $endtime);

                            //print_r($boobkly_appts); die;

                            // if(empty($boobkly_appts))
                            // {
                            //     $html .= '<option data-start="'.strtotime($start_date.' '.$slot['start']).'" data-end="'.$endtime.'" value="' . $slot['start'].'-'.$endtimestring. '" >' . $slot['start'].'-'.$endtimestring. '</option>';
                            // }
                            // else
                            // {
                            //     $start_time = strtotime($start_date.' '.$slot['start'])-($instructor->lesson_travel_time*60);

                            //     $is_blocked = 0;
                            //     foreach($boobkly_appts as $appts)
                            //     {
                            //         $appts_array = explode('-', $appts->time_slot);
                            //         $satrt_1 = $appts_array[0];
                            //         $end_1 = $appts_array[1];

                            //         $start = strtotime($start_date.' '.$satrt_1);
                            //         if($appts->type=="test")
                            //             { $start = $start-3600; }
                ...
                foreach($slots as $slot) {
                    if ($slot->is_blocked) {
                        $html .= '<option disabled value="' . $slot['start'].'-'.date('h:i a', strtotime($start_date.' '.$slot['end'])). '">' . $slot['start'].'-'.date('h:i a', strtotime($start_date.' '.$slot['end'])). '</option>';
                    } else {
                        $html .= '<option data-start="'.strtotime($start_date.' '.$slot['start']).'" data-end="'.strtotime($start_date.' '.$slot['end']).'" value="' . $slot['start'].'-'.date('h:i a', strtotime($start_date.' '.$slot['end'])). '" >' . $slot['start'].'-'.date('h:i a', strtotime($start_date.' '.$slot['end'])). '</option>';
                    }
                }
                return response()->json(['html' => $html, 'date'=>date( 'l, F d, ', strtotime($request->start_date)), 'boobkly_appts' => $boobkly_appts, 'av_time' => $av_time, 'slots' => $slots]);
            }
        }else{
