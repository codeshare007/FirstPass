...
@section('content')
    ...
    <div class="button_fix_area">
        <a href="{{ url('book-online/'.$search_id.'/instructor/'.$instructor->id) }}" class="btn btn-block btn-warning text-uppercase">Book Now</a>
    ...
    <section class="ls teaser-box-section container-cr">
        <div class="row c-gutter-8">
            <div class="col-sm-7 details_left">
                <div class="py-45 box-shadow mb-20 teaser">
                    <div class="col-md-12">
                        <div class="row">
                            ...
                            <?php
                            if($instructor->gender=="female"){ $gender = '<small><i class="fa fa-venus text-light-blue"></i></small>'; }
                            elseif($instructor->gender=="male"){ $gender = '<small><i class="fa fa-mars text-light-blue"></i></small>'; }
                            else{ $gender = ''; } ?>

                            <div class="luke_sec col-md-7">
                                <h5 class="title pull-left">{{ $Iname }} <?php echo $gender; ?></h5>

                                <span class="badge badge-warning pull-right">
                                    @if($instructor->user_meta->transmission_type == 'both')
                                        {{ ucfirst($search->t_type ?? '') }} Transmission
                                    @else
                                        {{ ucfirst($instructor->user_meta->transmission_type ?? '') }} Transmission
                                    @endif
                                </span>
                                ...
                                <div>
                                    <?php /*
                                    <div class="text-info mb-1"><i class="fa fa-mobile fa-fw fa-lg"></i> {{ $instructor->phone }}</div>
                                    ...
                                    */
                                    ...
                                    //echo "<pre>";
                                    //print_r($instructor_detail);
                                    ?>

                                    @if(@$instructor->wwccLicence->wwcc_licence_status == 'Approved')
                                        <div class="text-info mb-1">
                                            <i class="fa fa-check fa-fw fa-lg"></i> Working with Children
                                        </div>
                                    @endif

                                    @if($instructor->user_meta->years_for_instructing!='')
                                        <div class="text-info mb-1">
                                ...
                                <ul class="list-unstyled">
                                    <?php if($instructor->user_meta->association_member!="" || $instructor->user_meta->association_member!="No")
                                    {
                                        ...
                                        <li class="small-margin-top-10">
                                            <i class="fa fa-certificate fa-fw fa-lg"></i> Member of <?php echo $asso_name; ?>
                                        </li><?php
                                    } ?>
                                    @if($instructor->user_meta->keys2drive=='true')
                                        <li class="small-margin-top-10">
                                            <i class="fa fa-check fa-fw fa-lg"></i> Accredited for the 'Keys2Drive' program
                                        </li>
                                    @endif

                                    <?php
                                    if(!empty(@json_decode($instructor->user_meta->services_accreditation)))
                                    {
                                        foreach (@json_decode($instructor->user_meta->services_accreditation) as $value)
                                        {
                                            if($value==1)
                                            { ?>
                                                <li class="small-margin-top-10">
                                                    <i class="fa fa-asterisk fa-fw fa-lg"></i> Driving test package: existing customers
                                                </li> <?php
                                            }
                                            if($value==2)
                                            { ?>
                                                <li class="small-margin-top-10">
                                                    <i class="fa fa-asterisk fa-fw fa-lg"></i> Driving test package: new customers
                                                </li> <?php
                                            }
                                        }
                                    } ?>
                ...
                <div class="box-shadow mb-20 teaser">
                    <div class="container-cr p-3">
                        ...
                        <hr class="m-2">
                        {{ $instructor->user_meta->bio ==''? 'Not provided' : $instructor->user_meta->bio }}
                    </div>
                </div>
            ...
            <div class="col-sm-5">
                <div class="p-3 box-shadow mb-20 teaser details_right">
                    <div class="col-md-12">
                        <div class="row">
                            <a href="{{ url('book-online/'.$search_id.'/instructor/'.$instructor->id) }}" class="btn btn-block btn-warning text-uppercase">Book Now</a>
                            <button type="button" data-id="{{$instructor->id}}" class="btn btn-block btn-success av-btn text-uppercase">Check Availability</button>
                            ...
                            <div class="mb-3 mt-3">
                                <div class="media">
                                    <div class="media-left">
                                        <div class="small-width-180px">
                                            <strong class="badge badge-warning">{{ floor($avg_review->avg) }}</strong> &nbsp;
                                            @for($i=0; $i < floor($avg_review->avg); $i++ )
                                                <i class="fa fa-star text-warning fa-lg"></i>
                                            @endfor
                                        </div>
                                    </div>
                                    <div class="media-body">
                                        <small class="small-fontsize-14 text-gray">
                                            {{ $avg_review->total }} ratings
                                            <meta content="{{$avg_review->total}}" itemprop="ratingValue">
                ...
                @if ($search->t_type == "auto")
                    @if(@$instructor->user_vehicle_auto->vehicle_image != "")
                        <div class="p-3 box-shadow mb-20 teaser">
                            <h6>{{ ucfirst($instructor->name) }}'s Vehicle</h6>
                            <div class="row">
                                <div class="col-md-12 col-sm-12 text-right">

                                    <span class="badge badge-warning pull-right trans_badge"> Auto Transmission </span>

                                    <span class="badge badge-warning pull-right ancap_rating"> {{ $instructor->user_vehicle_auto->ancap_rating }} Star <small>ANCAP</small></span>

                                    <img src="{{ asset('assets/images/cars/'.$instructor->user_vehicle_auto->vehicle_image) }}" alt="" class="img-responsive">
                                    <br>

                                    <span class="badge badge-warning reg_no_badge pull-left">{{ $instructor->user_vehicle_auto->registration_number }}</span>
                                    ...
                                    <span class="badge badge-warning pull-right name_year_badge">
                                        @if(isset($car_make->title) && $car_make->title!=''){{$car_make->title}}, @endif
                                        @if(isset($car_model->title) && $car_model->title!=''){{ $car_model->title }}@endif
                                        @if(isset($instructor->user_vehicle_auto->vehicle_year) && $instructor->user_vehicle_auto->vehicle_year!='') - {{ $instructor->user_vehicle_auto->vehicle_year }}@endif
                                    </span>
                ...
                @if ($search->t_type == "manual")
                    @if(@$instructor->user_vehicle_manual->vehicle_image != "")
                        <div class="p-3 box-shadow mb-20 teaser">
                            <h6>{{ ucfirst($instructor->name) }}'s Vehicle</h6>
                            <div class="row">
                                <div class="col-md-12 col-sm-12 text-right">

                                    <span class="badge badge-warning pull-right trans_badge"> Manual Transmission </span>

                                    <span class="badge badge-warning pull-right ancap_rating"> {{ $instructor->user_vehicle_manual->ancap_rating }} Star <small>ANCAP</small></span>

                                    <img src="{{ asset('assets/images/cars/'.$instructor->user_vehicle_manual->vehicle_image) }}" alt="" class="img-responsive">
                                    <br>

                                    <span class="badge badge-warning reg_no_badge pull-left">{{ $instructor->user_vehicle_manual->registration_number }}</span>

                                    <?php
                                    $car_make = \App\CarMake::find($instructor->user_vehicle_manual->vehicle_make);
                                    $car_model = \App\CarModel::find($instructor->user_vehicle_manual->vehicle_model);
                                    ?>
                                    ...
                                    <span class="badge badge-warning pull-right name_year_badge">
                                        @if(isset($car_make->title) && $car_make->title!=''){{$car_make->title}}, @endif
                                        @if(isset($car_model->title) && $car_model->title!=''){{ $car_model->title }}@endif
                                        @if(isset($instructor->user_vehicle_manual->vehicle_year) && $instructor->user_vehicle_manual->vehicle_year!='') - {{ $instructor->user_vehicle_manual->vehicle_year }}@endif
                                    </span>
                ...
                <select name="user_regions[]" multiple="" class="form-control" id="user_regions" style="display:none">
                    @foreach($user_regions as $regionx)
                        <?php $selected="" ; ?>
                           <?php $selected="selected" ; ?>

                        <option {{ $selected }} value="{{ $regionx->region_id }}">{{ $regionx->region_id }}</option>
                    @endforeach
                </select>
            </div>
        </div>
                        ...
                        <div class="container">
                            ...
                            <div class="row calendar-extra">
                                <div class="col-md-12">
                                    ...
                                    <a href="{{ url('book-online/'.$search_id.'/instructor/'.$instructor->id) }}" class="btn btn-block btn-warning text-uppercase">Book With {{ $instructor->name }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('script')
    ...
    <?php } ?>
        ...
        // sticky section ends here
            ...
            {
                $.ajax({
                    url:"{{ route('get_review_pages')  }}?page="+page+'&intructor={{$instructor->id}}',
