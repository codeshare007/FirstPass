...
$(document).ready(function() {
    ...
    var table =  $('#datatable_table').DataTable({
        ...
        "ajax": "{{ route('instructor.get_notifications') }}",
        "columns":[
            { "data": "id", name:'id' },
...
<script>
    $(document).on('click', '.notify_view_status', function (e) {
        ...
        $.ajax({
            url: "{{ url('/instructor/view-notification/') }}"+"/"+notificationId,
            // data: notificationId,
            contentType: false,
            processData: false,
            type: 'get',
            success: function (res) {
                ...
                if(res.success == true){
                    if(res.notify_type == 'instructor_licence' || res.notify_type == 'wwcc_licence' || res.notify_type == 'driving_licence'){
                        var goto_url = "{{url('/my-documents')}}/";
                    }else{
                        var goto_url = "{{url('/profile-and-vehicle')}}/";
                    }
